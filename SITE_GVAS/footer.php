<!-- footer -->
    <footer>
        <div class="footer_part1">
            <div class="aigle ">
                <div class="flex">
                    <img src="Gloire/Assets/img/icon_logo/logo-GVAS.png"
                    alt="icon_logo"
                    onerror="this.style.display='none';">

                    <h1 style="font-size: 20px;">Groupe vision d'Aigle Services</h1>
                </div>
                <p id="justP">
                    De référence, GVAS est une structure professionnelle composée d’un personnel de qualité agréé
                    par
                    l’État. Spécialisée dans la vente des produits et services, l'accompagnement professionnel à
                    travers des formations qualifiantes et certifiantes mais aussi dans un suivi quelconque en vue
                    d’un domaine professionnel donné.
                </p>
            </div>

            <div class="aigle">
                <h1>Notre localisation</h1>
                <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d416.72322374458474!2d11.9054355!3d-4.822363!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x1a60af0b2c6b78c9%3A0xc598de43a114b8f5!2sGroupe%20Vision%20D&#39;aigle%20Services!5e1!3m2!1sen!2scg!4v1781644406727!5m2!1sen!2scg"
                   width="100%" height="255" allowfullscreen="" loading="lazy"
                   referrerpolicy="no-referrer-when-downgrade">
                </iframe>

                <a class="Hici" href="https://www.google.com/maps/place/Groupe+Vision+D'aigle+Services/@-4.822363,11.9054355,68m/data=!3m1!1e3!4m6!3m5!1s0x1a60af0b2c6b78c9:0xc598de43a114b8f5!8m2!3d-4.8224261!4d11.9054924!16s%2Fg%2F11npdsmz7k?entry=ttu&g_ep=EgoyMDI2MDYxMy4wIKXMDSoASAFQAw%3D%3D"
                    target="_blank">
                    " Cliquez ici pour Agrandir la MAPS "
                </a>
            </div>

            <div class="hide">
                <h1>Suivez nous sur nos pages</h1>
            </div>
            <div id="sous_class_contact">
                <div class="a">
                    <a href="../index.php">
                        <i class="fa-solid fa-house" style="color: rgb(0, 0, 0);"></i>
                    </a>
                </div>

                <div class="a">
                    <a href="https://www.facebook.com/profile.php?id=61583138888176" target="_blank">
                        <i class="fa-brands fa-square-facebook" style="color: rgb(0, 0, 0);"></i>
                    </a>
                </div>

                <div class="a">
                    <a href="#">
                        <i class="fa-brands fa-youtube" style="color: rgb(0, 0, 0);"></i>
                    </a>
                </div>

                <div class="a">
                    <a href="https://wa.me/242064201415" target="_blank">
                        <i class="fa-brands fa-whatsapp" style="color: rgb(0, 0, 0);"></i>
                    </a>
                </div>
            </div>

        </div>

        </div>

        <div class="footer_part2">
            <div class="pat">
                <h1>Nos pages</h1>
                <ul>
                    <li><a href="../index.php">Accueil</a></li>
                    <li><a href="../Formation.php">Formations</a></li>
                    <li><a href="../Vivi/apropos.php">A-Propos</a></li>
                    <li><a href="../Gloire/page_publicitaire.php">Page Publicitaire</a></li>
                    <li><a href="../galerie.php">Galerie</a></li>
                    <li><a href="../contacts.php">Contacts</a></li>
                    <li><a href="../temoignage.php">Témoignage</a></li>
                    <li><a href="../notre_image.php">Pourquoi GVAS ?</a></li>
                </ul>
            </div>

            <div class="pat">
                <h1>Nos Services</h1>
                <ul>
                    <li><a href="../Formation.php">Langues</a></li>
                    <li><a href="../Formation.php">Informatique</a></li>
                    <li><a href="../Formation.php">Pôle Commercial</a></li>
                </ul>
            </div>


            <div class="pat">
                <h1>Nos Contacts</h1>
                <ul>
                    <li>info@gvas.fr</li>
                    <li>(+242) 05 344 40 20</li>
                    <li>(+242) 06 420 14 15</li>
                    <li>(+242) 06 416 36 21</li>
                </ul>
            </div>

            <div class="form">
                <form action="../Traitement.php" method="POST">
                    <input type="hidden" name="form_type" value="commentaire">
                    <label for="envoie">
                        <h1 class="hid">
                            Envoyez votre commentaire
                        </h1>
                    </label>
                    <div>
                        <input class="tim" placeholder="Votre commentaire ici" type="text" name="envoie" id="envoie" required>
                        <input class="tom" type="submit" value="soumettre !"></input>
                    </div>
                </form>

                <div id="sous_class_contact">
                    <div class="a">
                        <a class="b" class="card1" href="../index.php">
                            <i class="fa-solid fa-house" style="color: rgb(0, 0, 0);"></i>
                        </a>
                    </div>

                    <div class="a">
                        <a class="b" class="card2" href="https://www.facebook.com/profile.php?id=61583138888176" target="_blank">
                            <i class="fa-brands fa-square-facebook" style="color: rgb(0, 0, 0);"></i>
                        </a>
                    </div>

                    <div class="a">
                        <a class="b" class="card3" href="#">
                            <i class="fa-brands fa-youtube" style="color: rgb(0, 0, 0);"></i>
                        </a>
                    </div>

                    <div class="a">
                        <a class="b" class="card4" href="https://wa.me/242064201415" target="_blank">
                            <i class="fa-brands fa-whatsapp" style="color: rgb(0, 0, 0);"></i>
                        </a>
                    </div>
                </div>
            </div>
        </div>

        <div class="bas">
            <h1>
                #celui qui ne se forme pas, se déforme !
            </h1>
        </div>
    </footer>

    <style>
        
/* footer */
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
  font-family: Cambria, Cochin, Georgia, Times, "Times New Roman", serif;
}

footer {
    overflow-x: hidden;
}


#sous_class_contact i {
  font-size: 30px;
}

#sous_class_contact .a
a {
  font-size: 20px;
  display: flex;
  justify-content: center;
  align-items: center;
  border: 5px solid white;
  border-radius: 50%;
  height: 45px;
  width: 45px;
  background-color: rgb(175, 173, 173);
}

#sous_class_contact div:hover {
  background-color: white;
  border-radius: 50%;

  transform: scale(1.1);
  box-shadow:
    0 5px 10px rgb(255, 251, 0),
    0 5px 10px rgb(255, 251, 0);
  cursor: pointer;
}

#sous_class_contact {
  gap: 15px;
  display: flex;
  justify-content: center;
  align-items: center;
}

.jaune {
    background-image: linear-gradient(to right,
      rgb(235, 253, 134),
      rgb(255, 255, 104));
    padding: 7px;
    width: 100%;
}

.jaune h1 {
  color: black;
}

.parie_publicitaire h1 {
  color: black;
}

#foot {
  color: var(--couleur-lien1);
  text-decoration: none;
  transition: 0.5s;
  background-image: linear-gradient(to right,
      rgb(235, 253, 134),
      rgb(255, 255, 104));
  padding: 7px;
  width: 100%;
}

#foot h1 {
  color: black;
}

#foot a {
  text-decoration: none;
  display: flex;
  justify-content: space-between;
  align-items: center;
}

#foot img {
  width: 50px;
  height: 50px;
  border-radius: 50%;
}

footer {
  background-image:
    radial-gradient(circle at 99% 33%,
      hsla(235, 0%, 48%, 0.03) 0%,
      hsla(235, 0%, 48%, 0.03) 37%,
      transparent 37%,
      transparent 100%),
    radial-gradient(circle at 46% 16%,
      hsla(235, 0%, 48%, 0.03) 0%,
      hsla(235, 0%, 48%, 0.03) 43%,
      transparent 43%,
      transparent 100%),
    radial-gradient(circle at 99% 25%,
      hsla(235, 0%, 48%, 0.03) 0%,
      hsla(235, 0%, 48%, 0.03) 22%,
      transparent 22%,
      transparent 100%),
    radial-gradient(circle at 57% 88%,
      hsla(235, 0%, 48%, 0.03) 0%,
      hsla(235, 0%, 48%, 0.03) 86%,
      transparent 86%,
      transparent 100%),
    radial-gradient(circle at 82% 78%,
      hsla(235, 0%, 48%, 0.03) 0%,
      hsla(235, 0%, 48%, 0.03) 22%,
      transparent 22%,
      transparent 100%),
    linear-gradient(90deg, rgb(0, 0, 0), rgb(0, 0, 0));
  color: var(--couleur-titre2);
  padding: 15px;
}

.footer_part2 {
  display: flex;
  flex-wrap: wrap;
  justify-content: space-evenly;
  gap: 25px;
  padding: 10px 10px 10px 10px;
}

.footer_part2 form {
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  gap: 10px;
  padding-bottom: 25px;
}

.footer_part2 form .tim {
  padding: 12px;
  border-radius: 8px;
  border: 1px solid gray;
  height: 50px;
  width: 250px;
  color: rgb(0, 0, 0);
}

.footer_part2 div .tom {
  background-image: linear-gradient(to right, rgb(61, 59, 59), rgb(235, 253, 134), rgb(243, 243, 36));
  padding: 12px;
  border-radius: 8px;
  border: 1px solid gray;
  height: 50px;
  width: 250px;
  color: rgb(0, 0, 0);
}

.footer_part2 form div {
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  gap: 10px;
}

.footer_part2 li {
  list-style-type: none;
  width: auto;
}

.footer_part2 .pat {
  width: auto;
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
}

.aigle {
  display: flex;
  justify-content: center;
  align-items: center;
  flex-direction: column;
  width: auto;
  gap: 10px;
}

.aigle,
p {
  height: 60%;
  width: 60%;
}

.aigle a,
h1 {
  color: var(--couleur-titre2);
  text-align: center;
  justify-content: center;
}

.footer_part1 {
  display: flex;
  justify-content: space-between;
  align-items: center;
  border-bottom: 5px solid white;
  width: 100%;
  padding: 10px 10px 20px 10px;
  gap: 20px;
}

.footer_part1 iframe {
  border: 1px solid white;
  border-radius: 20px;
  transition: 0.4s ease;
  box-shadow:
    0 5px 10px rgba(255, 255, 255, 0.1),
    0 15px 30px rgba(255, 255, 255, 0.2);
}

.aigle iframe:hover {
  transform: scale(1.01);
  box-shadow:
    0 5px 10px rgb(187, 187, 177),
    0 5px 10px rgb(192, 192, 178);
  cursor: pointer;
  border-radius: 1px;
}

.flex {
  display: flex;
  justify-content: center;
  align-items: center;
}

.flex img {
  height: 100px;
}

footer h1 {
  font-size: 30px;
  color: rgb(250, 250, 148);
}

p {
  font-size: 1em;
}

footer a:hover {
  color: yellow;
}

footer a {
  font-size: 1.2em;
  text-decoration: none;
  color: var(--couleur-lien2);
}

.footer_part1 #sous_class_contact {
  display: none;
}

.hide {
  display: none;
}

.form .sous_class_contact {
  display: flex;
  justify-content: center;
  align-items: center;
}

.g {
    width: 9%;
    position: absolute;
    margin-top: -4.4%;
}




@media (max-width: 1366px) {
  .ad-track {
    gap: 5px;
  }

  .baba1,
  .baba3 {
    display: none;
  }

  .Acceuil p {
    text-align: center;
  }

  .carouselExampleSlidesOnly2 #lien {
    margin-bottom: 15px;
    display: flex;
    justify-content: center;
    align-items: center;
  }
}

@media (max-width: 820px) {
  * {
    margin: 0;
    padding: 0;
  }

  .defilements h1 {
    font-size: 45px;
  }
  .defilements h2 {
    font-size: 25px;
  }

  .partie_navbar nav .liNone {
    display: flex;
  }

  .aigle .flex {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    width: 35% !important;
  }
  
    .aigle .flex h1 {
        white-space: wrap;
  }

  .footer_part1 {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
  }

  .footer_part1 p {
    text-align: center;
    width: 100%;
  }

  .footer_part1 h1 {
    width: 400px;
  }

  .Acceuil img {
    width: 300px;
  }

  .partie_recrutement {
    margin: 5px 0 15px 0;
  }

  .sous_class_logo h1 {
    font-size: 1.5em;
  }

  .defilements {
    height: 400px;
  }

  form {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
  }

  form label {
    font-size: 1.2em;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
  }

  form ul {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    gap: 10px;
  }

  .footer_part2 .pat {
    display: none;
  }

  /* .partenaires .float {
  border-radius: 10px;
  height: 150px;
  width: 100px;
}

.carouselExampleSlidesOnly2 #lien .partenaires {
  gap: 40px;
  display: flex;
  flex-direction: row;
}

.carouselExampleSlidesOnly2 div img {
  height: 80px;
  width: 80px;
}

.carouselExampleSlidesOnly2 #lien h1 {
  font-size: 0.5rem;
  z-index: 11;
} */

  .Acceuil .cent p {
    text-align: center;
    font-size: 15px;
    width: 100%;
  }

  .Acceuil .cent h1 {
    text-align: center;
    font-size: 20px;
    width: 300px;
  }

  .footer_part1 #sous_class_contact {
    display: flex;
    visibility: visible;
  }

  /* cacher les icônes sociales */
  #sous_class_contact {
    display: none;
  }

  .burger {
    display: flex;
    margin-left: auto;

  }

  #sous_class_contact {
    display: none;
  }

  #menu {
    display: none;
    position: absolute;
    top: 90px;
    right: 20px;
    background: rgb(122, 121, 121);
    width: 220px;
    padding: 20px;
    border-radius: 12px;
    box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15);
  }

  #menu.active {
    display: block;
  }

  .contacter {
    display: none;
  }

  #menu ul {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    gap: 20px;
    list-style: none;
    padding: 0;
    margin: 0;
  }

  #menu li {
    width: 100%;
    text-align: center;
  }

  #menu a {
    display: block;
    width: 100%;
    text-align: center;
  }

  .sous_class_logo h1 {
    width: 100%;
    font-size: 25px;
    text-align: left;
  }

  .flouter {
    height: 75px;
  }

  .section_navbar {
    height: 75px;
  }

  .sous_class_logo {
    width: 75%;
  }

  .redirections .bg {
    width: 330px;
  }

  .menu {
    width: 35%;
  }

  .menu button {
    width: 100px;
  }

  .menu a {
    width: 100px;
  }

  .pat {
    display: none;
  }

  .footer_part1 #sous_class_contact {
    display: flex;
  }

  .bas h1 {
    font-size: 20px;
  }

  .aigle h1 {
    font-size: 20px;
  }

  .aigle P {
    width: 300px;
  }

  .rondeurs {
    width: 320px;
  }

  #container {
    width: 100%;
  }
}

@media (min-width: 414px) and (max-width: 991px)  {
  #foot h1 {
    font-size: 15px;
  }

  .foo h2 {
    font-size: 15px;
  }

  .footer_part2 .pat {
    display: none;
  }

  #contenu {
    height: 50%;
    width: 100%;
  }

  .defilements h1 {
    font-size: 30px;
  }

   .defilements h2 {
    font-size: 15px;
  }

  .menu {
    width: 100%;
    height: 50%;
  }

  body {
    overflow-x: hidden;
  }

  .aigle .Hici {
    font-size: 24px;
    text-align: center;
  }

  .burger {
    display: flex;
  }

  .contacter {
    display: none;
    visibility: hidden;
  }

  #container .menu {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    width: 100%;
    height: 35%;
    padding: 15px;
  }

  #container {
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
    height: auto;
  }

  #container .menu #display {
    display: flex;
    flex-direction: row;
    justify-content: center;
    align-items: center;
    gap: 27px;
  }

  #contacter .menu #display button {
    margin-bottom: 15px;
  }

  .flexy {
    gap: 25px;
    display: flex;
    flex-direction: column;
    justify-content: center;
    align-items: center;
  }

  .contenu {
    height: 100%;
    width: auto;
  }
}

@media (max-width: 375px) {
  #justP {
    width: 200px;
  }

  .redirections .bg {
    width: auto !important;
  }
}

@media (max-width: 414px) {
  .partie_navbar nav ul {
    display: flex;
    flex-direction: column;
    gap: 10px;
    background-color: rgba(0, 0, 0, 0.3);
    position: absolute;
    top: 100%;
    left: 0;
    width: 100%;
    padding: 10px;
    border-radius: 5px;
    display: none;
  }

  #sous_class_contact {
    display: none;
    visibility: hidden;
  }

  .sous_class_logo img {
    display: block;
    visibility: visible;
  }

  .section_pub,
  .ad-banner,
  .ad-item {
    display: block;
    height: 2px;
  }

  .ad-track div {
    display: none;
  }

  .Acceuil img {
    width: 150px;
  }

  .Acceuil h1 {
    font-size: 15px;
  }

  .footer_part1 h1 {
    font-size: 25px;
  }

  .footer_part2 h1 {
    font-size: 25px;
  }

  form input {
    width: 250px;
  }

  .footer_part2 {
    display: flex;
    justify-content: center;
  }

  .footer_part2 {
    display: flex;
    flex-wrap: wrap;
    justify-content: center;
    gap: 25px;
    padding: 10px 10px 10px 10px;
  }

  .aigle P {
    text-align: center;
    width: 350px;
  }

  .redirections .bg {
    width: 350px;
  }
  
  
  .sous_class_contact2 {
    display: block;
    visibility: visible;
  }

  body {
    width: 100%;
  }
}

    </style>